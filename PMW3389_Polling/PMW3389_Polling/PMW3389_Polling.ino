...
void setup() {
  ...
  int maxSpiFreq = 2000000;
  SPISettings spiSettings(maxSpiFreq, MSBFIRST, SPI_MODE3);
...
void loop() {
  ...
  loop_counter ++;
    ...
    byte xl = adns_read_reg(Delta_X_L);
    byte xh = adns_read_reg(Delta_X_H);
    byte yl = adns_read_reg(Delta_Y_L);
    byte yh = adns_read_reg(Delta_Y_H);

    int dx = convTwosComp16(((unsigned int)xl) | (((unsigned int)xh) << 8));
    int dy = convTwosComp16(((unsigned int)yl) | (((unsigned int)yh) << 8));
